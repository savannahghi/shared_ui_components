...
showAlertSnackBar({
  ...
  Scaffold.of(context)
    ..hideCurrentSnackBar()
    ..showSnackBar(SilSnackBar(
      type: type ?? SnackBarType.danger,
      onAction: action,
      title: title,
      content: Text(
...
class SilSnackBar extends StatefulWidget implements SnackBar {
  ...
  @override
  SilSnackBar withAnimation(Animation<double> newAnimation, {Key fallbackKey}) {
    return SilSnackBar(
      key: key ?? fallbackKey,
      content: content,
...
class _SilSnackBarState extends State<SilSnackBar> {
  ...
  Widget build(BuildContext context) {
    ...
    Map<String, dynamic> snackBarMetaData = _snackBarMetaData();

    final Color backgroundColor = snackBarMetaData['color'];

    /// holds snackbar title, icon and action button
    ///
    Widget snackBarHeader = Container(
